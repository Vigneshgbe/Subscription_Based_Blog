...
if (!$article) {
    setFlashMessage('Article not found.', 'danger');
    header('Location: articles.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        setFlashMessage('Article updated successfully!', 'success');
        header('Location: articles.php');
        exit;
    }
...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        #content{min-height:350px;font-family:monospace;font-size:14px}
...
document.getElementById('mt_count').textContent = metaTitle.value.length;
document.getElementById('md_count').textContent = metaDesc.value.length;
metaTitle.addEventListener('input', () => document.getElementById('mt_count').textContent = metaTitle.value.length);
metaDesc.addEventListener('input',  () => document.getElementById('md_count').textContent = metaDesc.value.length);
...
function insertLink() {
    ...
    ta.value = ta.value.substring(0,s) + `<a href="${url}">${text}</a>` + ta.value.substring(e);
}
</script>
</body>
</html>